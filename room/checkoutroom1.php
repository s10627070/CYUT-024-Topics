...
$sql="update room set sum='已退房' where type='豪華房' and UserName='$user1'";
if (mysqli_query($dblink, $sql))
{
    $sql="UPDATE berry SET pass=null WHERE num=1;"; //退房清空房門密碼
    if(mysqli_query($dblink, $sql))
    {
        $sql="SELECT state FROM berry WHERE num=1;"; 
        $result = mysqli_query($dblink,$sql);
        $row = $result->fetch_assoc();
        $state = @$row['state'];
        if($state == TRUE) // if door is opened
        {
            ...
            $stream = ssh2_exec($connect,'python3 closedoor.py 1');// exec close door
            mysqli_query($dblink, "UPDATE berry SET state=False WHERE num=1;");
        }
