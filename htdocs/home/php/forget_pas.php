...
if($email!=$in_email){//該郵箱尚未註冊！ 
    ...
    $token = sha1($row['id'].$row['username'].$row['password']);
    $url = "https://mountain101.ddns.net/home/getmail.php?email=".$email."&token=".$token;//構造URL 
...
{  
    $update_result = update_time($UserId,$getpasstime);
        if($update_result)
        {
            echo 'yes';
        }
