...
<html>
	<head>
		...
		<link rel="stylesheet" href="../MJ/assets/css/main.css" />
	...
	<body>
		...
		<!-- Header -->
			<header id="header" class="alt">
				<div class="logo"><a href="../MJ/index.php">首頁 <span>by Siri & Jay</span></a></div>
				<a href="#menu">Menu</a>
			</header>
		<!-- Nav -->
			<nav id="menu">
				<ul class="links">
					...
					$sql = "SELECT pay FROM room where type='一般房' and UserName= '$user1'";
					...
					echo '<li><a href="../generic.html">l登入</a></li>';
					...
					if($pay=='已付款'){
						echo '<li><a href="checkoutroom2.php">退房</a><li>';
					}
					else{
		...
		<!-- 這裡播放房間圖片要嘛？ -->
			...
			<section class="banner full">
				<article>
					<img src="../MJ/images/room2.jpg" alt="" />
					<div class="inner">
						<header>
							<p>這裡可以做介紹</p>
							<h2>一般房</h2>
						...
						<div>
							<div class="box">
								...
								<div class="content">
									...
									$user1=$_SESSION["uname"];
									include_once ('../dbset.inc.php');
									$sql = "SELECT type,UserName,sum,phone,date1,date2,pay FROM room where type='一般房' and UserName= '$user1' and sum='已訂房'";
									$result = mysqli_query($dblink, $sql);
									...
									$sql = "SELECT RtnCode,roomid FROM ecpay where RtnCode='1' and roomid='$roomid'";
									$result = mysqli_query($dblink, $sql);
									$payrow = $result->fetch_assoc();
									$RtnCode=@$payrow['RtnCode'];
									/*$sql = "SELECT pass,num FROM berry WHERE num='2'";
									$result = mysqli_query($dblink, $sql);
